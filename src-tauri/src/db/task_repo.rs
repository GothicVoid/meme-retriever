...
pub async fn insert_task(pool: &DbPool, id: &str, file_path: &str) -> anyhow::Result<()> {
    ...
    .bind(id).bind(file_path).bind(now).bind(now)
    .execute(pool)
    .await?;
    Ok(())
}
...
pub async fn update_task_status(
    ...
    let now = now_secs();
    sqlx::query(
        "UPDATE task_queue SET status=?1, error_message=?2, updated_at=?3 WHERE id=?4"
    )
...
pub async fn get_pending_tasks(pool: &DbPool) -> anyhow::Result<Vec<TaskRecord>> {
    ...
    .fetch_all(pool)
    .await?;
    Ok(rows.into_iter().map(|r| TaskRecord {
        id: r.get("id"),
...
pub async fn reset_stale_tasks(pool: &DbPool) -> anyhow::Result<()> {
    let now = now_secs();
    sqlx::query(
        "UPDATE task_queue SET status='pending', updated_at=?1 WHERE status='processing'"
    )
    .bind(now)
    .execute(pool)
    .await?;
...
mod tests {
    ...
    async fn test_update_task_completed(pool: SqlitePool) {
        insert_task(&pool, "task-1", "/tmp/a.jpg").await.unwrap();
        update_task_status(&pool, "task-1", "completed", None).await.unwrap();
        let pending = get_pending_tasks(&pool).await.unwrap();
    ...
    async fn test_update_task_failed_with_message(pool: SqlitePool) {
        insert_task(&pool, "task-1", "/tmp/a.jpg").await.unwrap();
        update_task_status(&pool, "task-1", "failed", Some("OCR timeout")).await.unwrap();
    ...
    async fn test_clear_task_queue(pool: SqlitePool) {
        ...
        update_task_status(&pool, "t1", "completed", None).await.unwrap();
        update_task_status(&pool, "t2", "failed", Some("err")).await.unwrap();
    ...
    async fn test_reset_stale_tasks(pool: SqlitePool) {
        insert_task(&pool, "t1", "/tmp/a.jpg").await.unwrap();
        update_task_status(&pool, "t1", "processing", None).await.unwrap();
